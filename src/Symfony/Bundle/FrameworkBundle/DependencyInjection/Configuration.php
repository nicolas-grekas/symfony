...
class Configuration implements ConfigurationInterface
{
    ...
    private function addExtensionAliasSections(ArrayNodeDefinition $rootNode): void
    {
        ...
        // key => [aliased extension, what "true" stands for, accepts a bare string of resources]
        foreach ([
            ...
            $node = $children->variableNode($key)->aliasOf($alias)->treatFalseLike(['enabled' => false])->treatTrueLike($treatTrueLike);
        ...
        // "cache" has no enabled flag, FrameworkBundle and SecurityBundle reference cache.system unconditionally
        $children->variableNode('cache')->aliasOf('cache')->end();
    }
