...
class PhpConfigReferenceDumpPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container): void
    {
        ...
        $resolveExtension = fn (string $alias): ?string => isset($trees[$alias]) ? $this->camelCase($alias).'Config' : null;
        foreach ($trees as $alias => $tree) {
            $appTypes .= \sprintf("\n * @psalm-type %s = %s", $this->camelCase($alias).'Config', ArrayShapeGenerator::generate($tree, $resolveExtension));
        }
