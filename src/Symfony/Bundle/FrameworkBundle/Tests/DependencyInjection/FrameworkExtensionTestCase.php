...
abstract class FrameworkExtensionTestCase extends TestCase
{
    ...
    {
        if (!class_exists(AddIdentityStampsMiddleware::class)) {
            $this->markTestSkipped('AddIdentityStampsMiddleware not available.');
        }

        $container = $this->createContainerFromFile('messenger_identity_stamps', [], true, false);
        $container->compile();
    ...
    public function testMessengerPropagateStampsMiddlewareIsSharedByAllBuses()
    {
        if (!class_exists(PropagateStampsMiddleware::class)) {
            $this->markTestSkipped('PropagateStampsMiddleware not available.');
        }
