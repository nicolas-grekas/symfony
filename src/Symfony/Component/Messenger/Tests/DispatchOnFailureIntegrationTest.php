...
class DispatchOnFailureIntegrationTest extends TestCase
{
    ...
    public function testFailingFailureMessageDoesNotLoop()
    {
        $bus = $this->createBus([DummyMessage::class]);

        try {
            $bus->dispatch($first = new DummyMessage('first'), [new DispatchOnFailureStamp($failure = new DummyMessage('failure'))]);
            $this->fail('The failure should have been reported.');
        } catch (HandlerFailedException) {
        }
