            ...
            foreach ($configs as $i => $config) {
                ...
                foreach ($aliases as $key => $node) {
                    ...
                    if (!$container->hasExtension($alias)) {
                        throw new LogicException(\sprintf('The "%s.%s" configuration is handled by the "%s" extension, which is not registered.', $name, $key, $alias).UndefinedExtensionHandler::getPackageSuggestion($alias, $container->hasParameter(UndefinedExtensionHandler::PACKAGES_PARAMETER) ? $container->getParameter(UndefinedExtensionHandler::PACKAGES_PARAMETER) : []));
                    }
