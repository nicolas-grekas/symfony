...
class MergeExtensionConfigurationPass implements CompilerPassInterface
{
    ...
    /**
     * Moves the values of the nodes declared with NodeDefinition::extension() to the configuration of the extension they alias.
     */
    private function forwardExtensionAliases(ContainerBuilder $container): void
    {
        foreach ($container->getExtensions() as $name => $extension) {
            ...
            foreach ($tree->getChildren() as $key => $child) {
                if ($child instanceof BaseNode && null !== $child->getAttribute('extension')) {
                    $aliases[$key] = $child;
                }
            }
            ...
            foreach ($configs as $i => $config) {
                ...
                foreach ($aliases as $key => $node) {
                    ...
                    $alias = $node->getAttribute('extension');
                    ...
                    $value = $node->normalize($config[$key]) ?? [];
