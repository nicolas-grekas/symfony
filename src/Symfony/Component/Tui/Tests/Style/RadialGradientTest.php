...
final class RadialGradientTest extends TestCase
{
    public function testCenterCellGetsFirstColor()
    {
        ...
        $codes = self::codes($g->resolve(5, 5));
    ...
    public function testAspectRatioMakesCirclesVisual()
    {
        ...
        $codes = self::codes($g->resolve(5, 3));
    ...
    public function testCenteredFactoryIsEquivalentToFromWithCenter()
    {
        ...
        $this->assertSame(self::codes($g1->resolve(6, 4)), self::codes($g2->resolve(6, 4)));
    }
    ...
    public function testFromAcceptsACenterOutsideTheArea()
    {
        ...
        $codes = self::codes(RadialGradient::from(['#ff0000', '#0000ff'], 5.0, -3.0)->resolve(4, 3));
    ...
    public function testFromExistingGradientKeepsUnspecifiedGeometry()
    {
        ...
        $this->assertSame(
            ...
            self::codes($stretched->resolve(6, 4)),
    ...
    {
        $g = RadialGradient::centered('#ff0000', '#0000ff');
        $before = self::codes($g->resolve(5, 5));

        RadialGradient::from($g, 0.0, 0.0, 1.0);

        $this->assertSame($before, self::codes($g->resolve(5, 5)));
    }

    public function testFromArrayWithoutGeometryUsesDefaults()
    {
        $this->assertSame(
            self::codes(RadialGradient::from(['#ff0000', '#0000ff'], 0.5, 0.5, 2.0)->resolve(6, 4)),
            self::codes(RadialGradient::from(['#ff0000', '#0000ff'])->resolve(6, 4))
        );
    }
    ...
    public function testResolveIsStableForAGivenSize()
    {
        ...
        $a = self::codes($g->resolve(8, 6));
        $b = self::codes($g->resolve(8, 6));
    ...
    public function testResolveRecomputesASizeItHasEvicted()
    {
        ...
        $first = self::codes($g->resolve(8, 6));
        $g->resolve(16, 6);

        $this->assertSame($first, self::codes($g->resolve(8, 6)), 'an evicted size must be recomputed identically');
    }
