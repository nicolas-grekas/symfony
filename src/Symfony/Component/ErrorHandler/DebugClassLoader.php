...
class DebugClassLoader
{
    ...
    public function checkAnnotations(\ReflectionClass $refl, string $class): array
    {
        ...
        foreach ($refl->getMethods() as $method) {
            ...
            if ($canAddReturnType = isset($this->patchTypes['force']) && false === strpos($method->getFileName(), \DIRECTORY_SEPARATOR.'vendor'.\DIRECTORY_SEPARATOR)) {
            ...
            if (null !== ($returnType = self::$returnTypes[$class][$method->name] ?? self::MAGIC_METHODS[$method->name] ?? null) && !$method->hasReturnType() && !($doc && preg_match('/\n\s+\* @return +(\S+)/', $doc))) {
                ...
                list($normalizedType, $returnType, $declaringClass, $declaringFile) = \is_string($returnType) ? [$returnType, $returnType, $ns, ''] : $returnType;
...
EOTXT;
    ...
    private function fixReturnStatements(\ReflectionMethod $method, ?string $returnType)
    {
        ...
        if (null !== $returnType && 'docblock' !== ($this->patchTypes['force'] ?? null)) {
